...
#include <iostream>
#include <fstream>
#include "MEM/MEMAlgo/interface/read_slha.h"
//#include "MEM/MEMAlgo/interface/read_slha_c.h"
...
/* GG ??? Invalid */
static SLHAReader slha;
// GG C Interface

/*
extern "C" {

void  SLHA_Open(const char * name) {
  string file_name = name;
  slha =  SLHAReader(file_name);
  if (file_name != "")
    slha.read_slha_file(file_name);
}

double SLHA_getBlockEntry( const char * blockName, int index,
			   double def_val) {

  string block_name = blockName;
  vector<int> indices;
  indices.push_back(index);
  return slha.get_block_entry( block_name, indices, def_val);
}
